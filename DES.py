...
Date: 2017-09-01
Function:
DESEncryption(plaintext, key) 
DESDecryption(ciphertext, key) 
'''
# coding: utf-8
import base64  
...
#不考虑每字节的第8位，将64位密钥减至56位。然后进行一次密钥置换。
...
# base64_text = base64.encodestring(Result)  
#        f.write(Result)
    base64_text = base64.b64encode(Result.encode())
    print(base64_text)              
#----------若输入文本的长度不是8的整数倍，即不是64字节的整数倍，用空格补全（此处解密出来的密文用的是每8bit转换为一个ascii码，所以生成的八位表示的字符）-------
    length = len(Result)
    if length % 8 != 0 :  
